...
from tools import ALL_TOOLS, registry, get_tool_schema_list, execute_tool
...
class FunctionCallingTester:
    ...
    def test_function_calling(self, prompt: str, tools: Optional[List[Dict]] = None,
        ...
        tool_schemas = get_tool_schema_list(tools)
        ...
        for round_num in range(1, max_rounds + 1):
            ...
            response, tool_calls, response_time = self._send_streaming_request(messages, tool_schemas)
        ...
        return {
            ...
            "total_rounds": len(all_tool_calls) // max(len(all_tool_calls), 1),
            "timing": {
    ...
    def _send_streaming_request(self, messages: List[Dict], tool_schemas: List[Dict]) -> Tuple[Optional[Dict], List[Dict], float]:
    ...
    def _merge_tool_call(self, all_tool_calls: List[Dict], chunk_tool_call: Dict):
        ...
        # Ollama 的 index 在 function 内部
        chunk_func = chunk_tool_call.get("function", {})
        index = chunk_func.get("index")
        ...
        if index is not None and index < len(all_tool_calls):
            ...
            # 合并 arguments（可能是增量字符串）
            if chunk_func.get("arguments"):
                if "arguments" not in func:
                    func["arguments"] = ""
                func["arguments"] += chunk_func["arguments"]
            ...
            # 新增 tool_call
            all_tool_calls.append(chunk_tool_call)
    ...
    def _execute_tool(self, tool_call: Dict) -> Dict:
        ...
        try:
            ...
            # 使用 execute_tool 执行（通过 point 字段直接调用绑定函数）
            result = execute_tool(matched_tool, arguments)
...
def main():
    ...
    if args.prompt:
        ...
        test_cases = [
            {
                "name": "测试保存用户输入",
                "prompt": "请保存以下数据到日志：用户ID为12345，操作类型为'登录'，时间是现在。分类为'user_action'，优先级为'high'。"
            },
            {
                "name": "测试保存模型响应",
                "prompt": "把这条模型响应记录到日志里：分类是'model_response'，数据包含{'response_type': 'text', 'length': 100}，优先级medium。"
            },
            {
                "name": "测试查询日志",
                "prompt": "查询一下最近保存的日志数据，限制返回5条记录。"
            },
            {
                "name": "测试获取统计信息",
                "prompt": "获取当前的工具调用统计信息。"
            },
            {
                "name": "测试多工具调用",
                "prompt": "先保存一条系统事件日志（分类system_event，数据{'event': 'test_started'}），然后查询所有日志，最后获取统计信息。"
            }
        ]
